...
```
## Aanpassingen 
ik heb nu mijn programma een beetje aangepast, nu kan je zien hoeveel je per smaak hebt besteld
``` python 
print("Welkom bij Papi Gelato")
hoorntjes = 0
bakjes = 0
...
def bon():
    ...
    totaalPrijsHoorntjes = prijsHoorntje * hoorntjes
    totaalPrijsBakjes = prijsBakje * bakjes
    totaalPrijs = totaalPrijsAarbei + totaalPrijsChoco + totaalPrijsMunt + totaalPrijsVanille + totaalPrijsHoorntjes + totaalPrijsBakjes + totaalPrijsSlagroom + totaalPrijsSprinkels + totaalPrijsCaramelSausHorn + totaalPrijsCaramelSausBak
    if int(smaakA) > 0:
...
def bonZakelijk(liter):
    ...
    btw = 0.09
    totaalPrijs = totaalPrijsLiterA + totaalPrijsLiterC + totaalPrijsLiterM + totaalPrijsLiterV
    btwTotaalPrijs = totaalPrijs * btw
    print("-----------['Papi Gelato']-----------")
    print("")
    if smaakA > 0:
...
```
## F1.5.02.O8
Ik heb nu weer mijn programma aangepast, munt uit het assortiment weg gehaald, tekst aangepast en wat prijzen veranderd
``` python
print("Welkom bij Papi Gelato")
hoorntjes = 0
bakjes = 0
...
Ik heb nu een return functie gebruikt, zodat de programma op het laatst altijd nog iets print:
``` python
def particulierOfZakelijk():
    vraagParticulierOfZakelijk = input("Bent u 1) particulier of 2) zakelijk? ")
    if vraagParticulierOfZakelijk == "particulier" or vraagParticulierOfZakelijk == "Particulier" or vraagParticulierOfZakelijk == "1":
